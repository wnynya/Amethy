...
async function parse(opkgpath) {
  ...
  if (paperplugin) {
    let paperpluginYamlString = await pkg
      .file('paper-plugin.yml')
      .async('string');
    let paperpluginYaml = YAML.parse(paperpluginYamlString);
    paperpluginYaml.version = version;
    await pkg.file('paper-plugin.yml', YAML.stringify(paperpluginYaml));
  }
