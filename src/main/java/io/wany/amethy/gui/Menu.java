...
public class Menu {
  ...
  public static final TranslatableComponent keyboardCat = Component.translatable("")
      .args(Message.parse("amethy-menu-title"));
  ...
  public static class Main {
    ...
    public static Inventory inventory(Player player) {
      ...
      inventory.setItem(10, ItemTool.meta(new Skull(player).toItemStack(),
          Message.parse("§fHello ", Message.formatPlayer(player, "§r{prefix}§r{displayname}§r{suffix}§r"), "§f!")));
      ...
      inventory.setItem(28,
          ItemTool.meta(Skull.CHARACTER_P_PURPLE.toItemStack(), Message.parse(Message.effect("#D2B0DD;개인 설정")),
              List.of(Message.parse("§7플레이어 개인 설정"), Message.parse("§r"), Message.parse("§c사용 불가"))));

      if (CucumberySupport.ENABLE && CucumberySupport.EXIST) {
        inventory.setItem(34,
            ItemTool.meta(Skull.CHARACTER_C_LIME.toItemStack(), Message.parse(Message.effect("#52EE52;큐컴버리 메뉴")),
                List.of(Message.parse("§7큐컴버리 메뉴"), Message.parse("§r"), Message.parse("§e열려면 클릭하세요!"))));
      }
  ...
  public static void stateButton(Inventory inventory, int index, String title, List<Component> lore,
      ItemStack titleItem, ItemStack stateItem) {
  ...
  public static void booleanStateButton(boolean bool, Inventory inventory, int index, ItemStack titleItem, String title,
      List<Component> lore) {
    if (inventory.getSize() < index + 9) {
      return;
    }
    if (bool) {
      ...
      inventory.setItem(index + 9,
          ItemTool.meta(new ItemStack(Material.LIME_DYE), Message.parse(Message.effect("&a" + title)), lore));
    } else {
      inventory.setItem(index, ItemTool.meta(titleItem, Message.parse(Message.effect("&c" + title)), lore));
      inventory.setItem(index + 9,
          ItemTool.meta(new ItemStack(Material.GRAY_DYE), Message.parse(Message.effect("&c" + title)), lore));
    }
  }
