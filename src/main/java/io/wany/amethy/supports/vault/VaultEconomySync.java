...
public class VaultEconomySync {
   ...
   * public static HashMap<OfflinePlayer, Double> lastPlayersBalances = new
   * HashMap<>();
   * public static ExecutorService syncExecutorService =
   ...
   * 
   * try {
   * ResultSet checkSet = statement.executeQuery("SELECT * FROM " + tableName +
   ...
   * " (uuid, name, balance) VALUES ('" + uuidString + "', '"
   ...
   * return;
   * }
   * Console.debug(Message.effect(VaultSupport.PREFIX +
   * "  Load economy database-sync module"));
   * ENABLED = true;
   * 
   ...
   * if (n == 0) {
   * Console.debug(Message.effect(VaultSupport.PREFIX +
   * "  Table not exist. create new table"));
   * statement.executeUpdate("CREATE TABLE " + tableName + " (" +
   * "uuid varchar(64) NOT NULL, "
   * + "name varchar(64) NOT NULL, " + "balance bigint NOT NULL," +
   * "PRIMARY KEY (`uuid`) "
   ...
   * @Override
   * public void run() {
   * HashMap<OfflinePlayer, Double> lbp = (HashMap<OfflinePlayer, Double>)
   * lastPlayersBalances.clone();
   ...
   * .executeQuery("SELECT balance FROM " + tableName + " WHERE uuid = '" +
   * player.getUniqueId() + "';");
   * resultSet.next();
