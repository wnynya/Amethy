...
import io.wany.amethy.modules.MySQLClient;
...
public class VaultEconomySync {
  ...
  public static MySQLClient database;
  ...
  public static void onPlayerJoin(PlayerJoinEvent event) {
    ...
    try {
      ...
      if (n == 0) {
        ...
        statement.executeUpdate("INSERT INTO " + tableName + " (uuid, name, balance) VALUES ('" + uuidString + "', '"
  ...
  public static void onEnable() {
    ...
    MySQLClient.DatabaseConfig databaseConfig = new MySQLClient.DatabaseConfig(
    ...
    tableName = Amethy.CONFIG.getString("vault-support.economy.database-sync.table");

    Console
        .debug(Message.effect(VaultSupport.PREFIX + "    Use database " + databaseConfig.database + "." + tableName));
    ...
    try {
      ...
      if (n == 0) {
        ...
        statement.executeUpdate("CREATE TABLE " + tableName + " (" + "uuid varchar(64) NOT NULL, "
            + "name varchar(64) NOT NULL, " + "balance bigint NOT NULL," + "PRIMARY KEY (`uuid`) "
    ...
    for (Player player : Bukkit.getOnlinePlayers()) {
      String uuidString = player.getUniqueId().toString();
      try {
        ResultSet checkSet = statement
            .executeQuery("SELECT * FROM " + tableName + " WHERE uuid = '" + uuidString + "';");
        int n = 0;
        while (checkSet.next()) {
          n++;
        }
        ...
        for (OfflinePlayer player : lbp.keySet()) {
          ...
          try {
            ResultSet resultSet = statement
                .executeQuery("SELECT balance FROM " + tableName + " WHERE uuid = '" + player.getUniqueId() + "';");
            resultSet.next();
          ...
          if (currentBalance != dbBalance) {
            try {
              statement.executeUpdate("UPDATE " + tableName + " SET balance = " + currentBalance + " WHERE uuid = '"
                  + player.getUniqueId() + "';");
            } catch (Exception e) {
              e.printStackTrace();
            }
